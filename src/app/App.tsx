...
function App() {
    ...
    const loadPostIts = async () => {
      try {
        ...
        if (data.length === 0) {
          ...
          try {
            await batchSavePostIts(INITIAL_POSTITS);
            console.log('✅ Initial data synced to backend');
          } catch (error) {
            console.log('⚠️ Could not sync initial data to backend, saved to localStorage');
          }
        } else {
          setPostIts(data);
        }
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToPostIts(
      // On INSERT
      (newPostIt) => {
        if (isUpdatingFromRealtime.current) return;
        console.log('📥 Received new post-it from Realtime:', newPostIt);
        setPostIts((prev) => {
          // Check if post-it already exists (avoid duplicates)
          if (prev.some(p => p.id === newPostIt.id)) {
            return prev;
          }
          const updated = [...prev, newPostIt];
          // Update localStorage
          localStorage.setItem('connectme_postits', JSON.stringify(updated));
          return updated;
        });
      },
      // On UPDATE
      (updatedPostIt) => {
        if (isUpdatingFromRealtime.current) return;
        console.log('📝 Received updated post-it from Realtime:', updatedPostIt);
        setPostIts((prev) => {
          const updated = prev.map(p => p.id === updatedPostIt.id ? updatedPostIt : p);
          // Update localStorage
          localStorage.setItem('connectme_postits', JSON.stringify(updated));
          return updated;
        });
      ...
      (deletedId) => {
        if (isUpdatingFromRealtime.current) return;
        console.log('🗑️ Received delete from Realtime:', deletedId);
        setPostIts((prev) => {
  ...
  // Auto-sync to backend every 30 seconds if data changed
  useEffect(() => {
  ...
  const handleCreatePostIt = async (newPostItData: Omit<PostIt, 'id' | 'position' | 'participants'>) => {
    ...
    try {
      ...
      console.log('💾 Saved new post-it to localStorage');
    } catch (error) {
    ...
    try {
      ...
      console.log('✅ Saved new post-it to backend');
    } catch (error) {
      console.error('⚠️ Failed to save new post-it to backend (saved to localStorage):', error);
      // Don't remove from UI - it's saved in localStorage
    }
  };
  ...
  const handleUpdatePostItPosition = async (id: string, x: number, y: number) => {
    ...
    // Mark that we're updating (to avoid processing our own Realtime events)
    isUpdatingFromRealtime.current = true;
    ...
    // Try to save to database
    try {
      console.log(`💾 Calling API to save position for post-it ${id}`);
      ...
      // Don't reload - keep using localStorage
    } finally {
      // Reset flag after a short delay
      setTimeout(() => {
        isUpdatingFromRealtime.current = false;
  ...
  const handleRemoveParticipation = async (postItId: string) => {
    // This is handled by the same handleParticipate function
    await handleParticipate(postItId);
  };
