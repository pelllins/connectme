...
export function PostItDetail({ postIt, onClose, onParticipate, userMatricola = '10123456' }: PostItDetailProps) {
  const [isLoading, setIsLoading] = useState(false);

  if (!postIt) return null;

  // Controlla se l'utente è già partecipe
  const isJoined = (postIt.participantIds || []).includes(userMatricola);
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6">
                ...
                <button 
                  ...
                  className={`w-full py-4 rounded-xl font-semibold transition-all border shadow-md ${
                    ...
                    isJoined 
                      ? 'bg-white text-slate-800 border-slate-200 hover:bg-slate-50 disabled:hover:bg-white' 
                      : 'bg-gradient-to-r from-[#8D7ED4] to-[#3CA9D3] text-white border-transparent hover:opacity-95 disabled:hover:opacity-100'
                  }`}
                >
                  {isLoading ? 'Elaborazione...' : (isJoined ? 'Disiscriviti' : 'Partecipa!')}
                </button>
              </div>
            </div>
        </motion.div>
